...
public class courseTable extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        InitialTheCourseTable(Coursetable,week);
    }
    ...
    public  void SetEmptyCourseTable(DisplayMetrics dm){
        ...
        for(int i = 1; i <= 10; i ++){
            ...
            for(int j = 1; j <= 8; j ++){
                ...
                TextView tx = new TextView(courseTable.this);
                tx.setId((i - 1) * 8  + j);
                //除了最后一列，都使用course_text_view_bg背景（最后一列没有右边框）
                if(j < 8)
                    tx.setBackgroundDrawable(courseTable.this.
        ...
        for(int i=0;i<Coursetable.size();i++){
            if(Coursetable.get(i).getWeeks().equals(week              )){
                ...
                if(Coursetable.get(i).getTime().equals("[01-02节]"))
                    ...
                    time=time;
                coursename=Coursetable.get(i).getCourseName();
                location='@'+Coursetable.get(i).getClassRoom();
